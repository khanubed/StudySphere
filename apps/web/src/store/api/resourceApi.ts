...
export interface ModerateResourcePayload {
  id: string;
  action: 'approve' | 'reject' | 'request_changes';
  feedback?: string;
}

// In-memory persistent array for mock mutations during the browser session
let dynamicResources: Resource[] = [...mockResourcesList];
let dynamicComments: Comment[] = [...mockCommentsList];
  ...
  endpoints: (builder) => ({
    ...
    >({
      queryFn: async (params) => {
        let items = dynamicResources.filter((r) => r.status === 'published');
        const filter = params || {};
        ...
        if (filter.sortBy === 'downloads') {
          items = [...items].sort((a, b) => (b.downloadsCount || 0) - (a.downloadsCount || 0));
        } else if (filter.sortBy === 'popular') {
          items = [...items].sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0));
        } else if (filter.sortBy === 'rating') {
          items = [...items].sort((a, b) => (b.bookmarksCount || 0) - (a.bookmarksCount || 0));
        } else {
          // 'latest'
          items = [...items].sort(
    ...
    getResourceById: builder.query<ApiResponse<Resource>, string>({
      queryFn: async (id) => {
        const resource = dynamicResources.find((r) => r.id === id);
        if (!resource) {
          return {
            error: {
              status: 404,
              data: { success: false, message: 'Resource not found in library catalogue' },
            } as any,
          };
        }
        return {
    ...
    validateDriveUrl: builder.mutation<ApiResponse<DriveValidationResult>, { url: string }>({
      queryFn: async ({ url }) => {
          ...
          return {
            data: {
              ...
              data: {
                ...
                mimeType: '',
                isPublic: false,
                error: 'Invalid Google Drive link format. Please paste a valid share link.',
              },
        ...
        // Simulate public file detection
        return {
          data: {
            success: true,
            data: {
            ...
            message: 'Google Drive file verified successfully with public view access',
          },
        };
      },
    }),
    ...
    submitResource: builder.mutation<ApiResponse<Resource>, SubmitResourcePayload>({
      queryFn: async (payload) => {
        const newResource: Resource = {
          ...
          tags: payload.tags || ['Curriculum', 'VerifiedSyllabus'],
          likesCount: 0,
          bookmarksCount: 0,
          downloadsCount: 0,
          commentsCount: 0,
        ...
        dynamicResources.unshift(newResource);

        return {
          data: {
            success: true,
            data: newResource,
            message: 'Resource submitted to academic moderation queue',
          },
        };
      },
      invalidatesTags: [{ type: 'Resource', id: 'LIST' }, { type: 'Resource', id: 'MY_RESOURCES' }, 'Dashboard'],
    }),
    ...
    getResourceComments: builder.query<ApiResponse<Comment[]>, string>({
      queryFn: async (resourceId) => {
        const comments = dynamicComments.filter((c) => c.resourceId === resourceId);
        return {
          data: {
            success: true,
            data: comments,
      ...
      queryFn: async ({ resourceId, content }) => {
        ...
        return {
          data: {
            success: true,
            data: newComment,
            message: 'Comment posted to resource ledger',
          },
        };
      },
    ...
    getMyResources: builder.query<ApiResponse<Resource[]>, { status?: ResourceStatus | 'all' } | void>({
      queryFn: async (params) => {
        ...
        let items = dynamicResources.filter((r) => r.uploadedBy === 'usr-student-01');
        if (status !== 'all') {
      ...
      queryFn: async (params) => {
        ...
        return {
          data: {
            success: true,
            data: entries,
            message: 'Contributor leaderboard loaded',
          },
        };
      },
    ...
    getResourceModerationQueue: builder.query<ApiResponse<Resource[]>, void>({
      queryFn: async () => {
        const items = dynamicResources.filter((r) => r.status === 'pending' || r.status === 'changes_requested');
        return {
          data: {
            success: true,
            data: items,
            message: 'Admin moderation queue loaded',
          },
        };
      },
      providesTags: [{ type: 'Resource', id: 'MODERATION_QUEUE' }],
    }),

    moderateResource: builder.mutation<ApiResponse<Resource>, ModerateResourcePayload>({
      queryFn: async ({ id, action, feedback }) => {
        const resource = dynamicResources.find((r) => r.id === id);
        if (!resource) {
          return {
            error: {
        ...
        if (action === 'approve') {
          ...
          resource.verifiedBy = 'Admin Lead';
          resource.moderationFeedback = undefined;
        } else if (action === 'reject') {
          resource.status = 'rejected';
          resource.rejectionReason = feedback;
          resource.moderationFeedback = feedback;
        ...
        return {
          data: {
            success: true,
            data: resource,
            message: `Resource marked as ${resource.status}`,
          },
        };
      },
      invalidatesTags: [
        { type: 'Resource', id: 'LIST' },
        { type: 'Resource', id: 'MODERATION_QUEUE' },
        { type: 'Resource', id: 'MY_RESOURCES' },
      ],
    }),

    getSidebarMetadata: builder.query<ApiResponse<{ popularSubjects: typeof mockPopularSubjects; trendingTags: string[] }>, void>({
      queryFn: async () => {
        return {
          data: {
            success: true,
            data: {
              popularSubjects: mockPopularSubjects,
